...
public class ValidationBus<STATE, STATE_WRAPPER extends AutoCloseable, IMMUTABLE_CHANGES extends ImmutableChanges, IMMUTABLE_STATE>
    ...
    public ValidationBus(final NonBlockingReactor _reactor) throws Exception {
        super(_reactor);
    }
    ...
    public AsyncRequest<String> sendAReq(final IMMUTABLE_CHANGES _changes) {
        return new AsyncBladeRequest<String>() {
            ...
            AsyncResponseProcessor<Void> notificationResponseProcessor = new AsyncResponseProcessor<Void>() {
                @Override
            ...
            protected void processAsyncRequest() throws Exception {
                setExceptionHandler(new ExceptionHandler<String>() {
                    @Override
                    public String processException(final Exception e)
                            throws Exception {
                ...
                final Iterator<Subscription<IMMUTABLE_CHANGES, Void>> it = subscriptions
                        .iterator();
                while (it.hasNext()) {
                    final ValidationSubscription<STATE, STATE_WRAPPER, IMMUTABLE_CHANGES, IMMUTABLE_STATE> subscription = (ValidationSubscription) it
                            .next();
                    final Validator<IMMUTABLE_CHANGES> validator = subscription.validator;
                    final String prefix = validator.getPrefix();
                    if (_changes.hasMatchingChange(prefix)) {
                        send(subscription.notificationAReq(_changes),
                                notificationResponseProcessor);
                    } else {
                        count--;
                    }
                }
